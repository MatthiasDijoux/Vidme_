      ...
      <v-row >
        <v-col cols="12">
          <v-card width="500px" class="px-5 justify-center">
            ...
            role="alert"
          >Vous n'êtes pas autorisé à accéder à ce room!</div>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn 
              icon 
...
export default {
  data() {
    return {
      ...
      roomToken: (this.$route.query.roomToken && this.$route.query.roomToken ) ? this.$route.query.roomToken : "",
      roomToCreate: '',
      errors: ''
    };
  },
  ...
  methods: {
        ...
        handdleRoomAccess() {
            ...
            if (this.pseudo != '') {
                const dataToSend = {roomToken: this.roomToken, room:this.room, pseudo:this.pseudo}
                ...
                axios.post(API_URL + 'access-room', dataToSend)
                .then(response => {
                  ...
                  if(response.data.roomId && response.data.roomId !== '') {
                    // this.sendInfo()
